...
def segment_metrics(pred_list, gt_list, num_cls=2):
    ...
    for v in counts:
        tp, fp, fn = v
        mean_acc_list.append(tp / (tp + fn) if (tp + fn) > 0 else 0.0)
        mean_iou_list.append(tp / (tp + fp + fn) if (tp + fp + fn) > 0 else 0.0)

    mean_acc = np.mean(mean_acc_list)
    mean_iou_acc = np.mean(mean_iou_list)
...
def prf_metrics(pred_list, gt_list):
    ...
    for pred, gt in zip(pred_list, gt_list):
        ...
        max_val = np.max(pred)
        if max_val <= 0:
            pred_img = np.zeros_like(pred, dtype='uint8')
        else:
            pred_img = ((pred / max_val) > 0.5).astype('uint8')
    ...
    p_acc = 1.0 if tp == 0 and fp == 0 else tp / (tp + fp) if (tp + fp) > 0 else 0.0
    r_acc = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    f_acc = 2 * p_acc * r_acc / (p_acc + r_acc) if (p_acc + r_acc) > 0 else 0.0
...
def cal_OIS_metrics(pred_list, gt_list, thresh_step=0.01):
    ...
    for pred, gt in zip(pred_list, gt_list):
        ...
        for thresh in np.arange(0.0, 1.0, thresh_step):
            ...
            p_acc = 1.0 if tp == 0 and fp == 0 else tp / (tp + fp) if (tp + fp) > 0 else 0.0
            r_acc = tp / (tp + fn) if (tp + fn) > 0 else 0.0
            F1 = 2 * p_acc * r_acc / (p_acc + r_acc) if (p_acc + r_acc) > 0 else 0.0
    ...
    final_F1 = np.mean(np.array(final_F1_list))
    return final_F1
    ...
    for thresh in np.arange(0.0, 1.0, thresh_step):
        ...
        for i, (pred, gt) in enumerate(zip(pred_list, gt_list)):
            ...
            if (FN + FP + TP) <= 0:
                ...
                iou_1 = TP / (FN + FP + TP) if (FN + FP + TP) > 0 else 0.0
                iou_0 = TN / (FN + FP + TN) if (FN + FP + TN) > 0 else 0.0
                iou = (iou_1 + iou_0) / 2
...
def eval(log_eval, results_dir, epoch):
    ...
    src_img_list, tgt_img_list, pred_imgs_names, gt_imgs_names = get_image_pairs(
        results_dir, suffix_gt, suffix_pred
    )
    ...
    thresh_list = final_accuracy_all[:, 0]
    Precision_list = final_accuracy_all[:, 1]
    Recall_list = final_accuracy_all[:, 2]
    F_list = final_accuracy_all[:, 3]

    # 取全局最佳 F1 对应的阈值、P、R、F1
    best_idx = np.argmax(F_list)
    best_thresh = float(thresh_list[best_idx])
    best_precision = float(Precision_list[best_idx])
    best_recall = float(Recall_list[best_idx])
    best_f1 = float(F_list[best_idx])

    mIoU = cal_mIoU_metrics(
        src_img_list, tgt_img_list,
    ...
    log_eval.info("BestThresh -> " + str(best_thresh))
    log_eval.info("F1 -> " + str(best_f1))
    log_eval.info("P -> " + str(best_precision))
    log_eval.info("R -> " + str(best_recall))
...
if __name__ == '__main__':
    ...
    print("ODS -> " + str(ODS))
    print("OIS -> " + str(OIS))
    print("BestThresh -> " + str(best_thresh))
    print("F1 -> " + str(best_f1))
    print("P -> " + str(best_precision))
    print("R -> " + str(best_recall))
    print("eval finish!")