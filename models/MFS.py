...
class MFS(nn.Module):
    def __init__(self, embedding_dim):
        ...
        # 🌟 BGF 组件：用于提取高频几何边界图
        # =====================================================================
        self.edge_conv1 = nn.Conv2d(embedding_dim, 1, kernel_size=3, padding=1)
        self.edge_conv2 = nn.Conv2d(embedding_dim, 1, kernel_size=3, padding=1)
    ...
    def forward(self, inputs):
        ...
        # =====================================================================
        # 🌟 BGF 执行逻辑：开启状态，强化深层边界 [cite: 40]
        # =====================================================================
        edge_map = self.edge_conv1(out_c1) + self.edge_conv2(out_c2)
        M_geo = torch.sigmoid(edge_map) 
        
        out_c3 = out_c3 * (1 + self.gamma * M_geo)
        out_c4 = out_c4 * (1 + self.gamma * M_geo)
        # =====================================================================
